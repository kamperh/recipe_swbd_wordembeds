...
"""
Create the segment files for the word segments used in the samediff evaluation.
...
def check_argv():
    ...
    parser.add_argument("testset_fn", type=str, help="original file list")
...
def main():
    ...
    for line in open(feats_scp_fn):
        ...
        # print utterance_segs[line]

    # Create word segments dict
    word_segs = {}  # word_segs["organized_sw02111-A_000280_000367"] is ("sw02111-A", 280, 367)
    for line in open(samediff_fn):
        word, _, conversation, _, start, end = line.split(" ")
        conversation = "sw0" + conversation[:-1] + "-" + conversation[-1]
        start = int(start)
        end = int(end)
        word_id = word + "_" + conversation + "_" + "%06d" % start + "-" + "%06d" % end
